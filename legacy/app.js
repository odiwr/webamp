...
(function () {
  ...
  function makeVerticalTranslateDrag({
    ...
    const move = (e) => {
      ...
      const nextY = clamp(Math.round(localY - handleRect.height / 2), minY, max);
...
// -----------------------------
  ...
  function init() {
    ...
    $('#equalizer-close')?.addEventListener('click', (e) => {
      e.preventDefault();
      setWindowVisible(eqWindow, false);
      eqButton?.classList.remove('selected');
    });

    wirePressState($('#playlist-close-button'));
    $('#playlist-close-button')?.addEventListener('click', (e) => {
      e.preventDefault();
      setWindowVisible(plWindow, false);
      plButton?.classList.remove('selected');
    });
